...
def replaceIP(initial, new_num):  # Call this function to replace data in '.txt' file
    newline = ""
    str_num = str(new_num)
...
def call_back(event):  # When this function is called,client commands the car to move backward
    global c_b_stu
    if c_b_stu == 0:
        tcpSocket.send(('backward').encode())
        c_b_stu = 1
...
def call_LeftSide(event):
    global c_ls_stu
    if c_ls_stu == 0:
        tcpSocket.send(('leftside').encode())
        c_ls_stu = 1
...
def call_RightSide(event):
    global c_rs_stu
    if c_rs_stu == 0:
        tcpSocket.send(('rightside').encode())
        c_rs_stu = 1


def call_headup(event):
    tcpSocket.send(('headup').encode())


def call_headdown(event):
    tcpSocket.send(('headdown').encode())


def call_headleft(event):
    tcpSocket.send(('low').encode())


def call_headright(event):
    tcpSocket.send(('hight').encode())


def call_headhome(event):
    tcpSocket.send(('headhome').encode())


def call_steady(event):
    if funcMode == 0:
        tcpSocket.send(('steady').encode())
    else:
        tcpSocket.send(('funEnd').encode())


def call_FindColor(event):
    if funcMode == 0:
        tcpSocket.send(('FindColor').encode())
    else:
        tcpSocket.send(('funEnd').encode())
...
def call_WatchDog(event):
    if funcMode == 0:
        tcpSocket.send(('WatchDog').encode())
    else:
        tcpSocket.send(('funEnd').encode())
...
def call_Smooth(event):
    global SmoothMode
    if SmoothMode == 0:
        tcpSocket.send(('Smooth_on').encode())
        SmoothMode = 1
    else:
        tcpSocket.send(('Smooth_off').encode())
        SmoothMode = 0
...
def call_Switch_1(event):
    global Switch_1
    if Switch_1 == 0:
        tcpSocket.send(('Switch_1_on').encode())
        Switch_1 = 1
    else:
        tcpSocket.send(('Switch_1_off').encode())
        Switch_1 = 0
...
def call_Switch_2(event):
    global Switch_2
    if Switch_2 == 0:
        tcpSocket.send(('Switch_2_on').encode())
        Switch_2 = 1
    else:
        tcpSocket.send(('Switch_2_off').encode())
        Switch_2 = 0
...
def call_Switch_3(event):
    global Switch_3
    if Switch_3 == 0:
        tcpSocket.send(('Switch_3_on').encode())
        Switch_3 = 1
    else:
        tcpSocket.send(('Switch_3_off').encode())
        Switch_3 = 0
...
def connection_thread():
    ...
    while 1:
        car_info = (tcpSocket.recv(BUFSIZ)).decode()
        if not car_info:
            continue
...
def socket_connect():  # Call this function to connect with the server
    global ADDR, tcpSocket, BUFSIZ, is_connected, ipaddr
    ...
    tcpSocket = socket(AF_INET, SOCK_STREAM)  # Set connection value for socket
    ...
    for i in range(1, 6):  # Try 5 times if disconnected
        ...
        if is_connected == 0:
            ...
            print("Connecting")
            tcpSocket.connect(ADDR)  # Connection with the server
...
def set_R(event):
    time.sleep(0.03)
    tcpSocket.send(('wsR %s' % var_R.get()).encode())


def set_G(event):
    time.sleep(0.03)
    tcpSocket.send(('wsG %s' % var_G.get()).encode())


def set_B(event):
    time.sleep(0.03)
    tcpSocket.send(('wsB %s' % var_B.get()).encode())


def loop():  # GUI
    global tcpSocket, root, E1, connect, l_ip_4, l_ip_5, color_btn, color_text, Btn14, CPU_TEP_lab, CPU_USE_lab, RAM_lab, canvas_ultra, color_text, var_R, var_B, var_G, Btn_Steady, Btn_FindColor, Btn_WatchDog, Btn_Fun4, Btn_Fun5, Btn_Fun6, Btn_Switch_1, Btn_Switch_2, Btn_Switch_3, Btn_Smooth  # The value of tcpSocket changes in the function loop(),would also changes in global so the other functions could use it.
    while True:
